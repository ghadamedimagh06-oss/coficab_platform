...
- [~] **W1.1 — Single optimizer (safe slice done).** ✅ The canonical engine is now explicitly `DailyPlanBuilder` (module header + per-class docstrings designate it; the two legacy classes carry LEGACY/SPECIAL-PURPOSE notes). Removed the **redundant third optimizer** `vrptw_complete_optimizer.VRPTWCompleteOptimizer` (a passthrough wrapper, no production callers) — 3 optimizer modules → 2; retargeted its 4 tests to the real `VRPTWOptimizer`. Guarded by the W4.3 golden tests. **Deliberately NOT done (out of scope, high-risk):** routing the DB-materialisation path (`VrptwOptimizer` → PlanVersion) and the dashboard Load-Efficiency KPI (`VRPTWOptimizer`, the ~85% number) through the builder — those have different I/O contracts (DB writes / a different utilisation model) and rewriting them would change the dashboard KPI. They stay as documented special-purpose engines. **(L)**
...
- [x] **W3.1 — C4 Agentic copilot write-tools.** ✅ DONE & tested. `services/copilot_actions.py` turns a plain-language instruction into a grounded PROPOSAL with a DETERMINISTIC intent parser (no LLM needed): **summary** (grounded plan read-out), **explain truck N** (reuses the explainable-routing engine), and **breakdown→replan** (a WRITE action: re-optimises the remaining stops on the remaining fleet and returns the recovery + diff for one-click approval). `POST /api/copilot/action` (works whether or not Groq is configured). Factored `compute_replan()` shared by `/replan` and the copilot (W1.1 spirit: no duplication). Frontend `CopilotActionBar` (command box + suggestions → proposal card → Apply recovery) mounted on generated-daily-planning. `tests/test_copilot_actions.py` (16 tests). **(L)**
...
- _2026-06-14_ — **W2.1 objective fix: coverage-first CO₂ portfolio.** Fixed the green-objective flaw the W4.3 golden tests surfaced: a single fixed-makespan solve could abandon a serviceable customer and mislabel CO₂ (green > fast km). Arc cost is now pure travel distance (service stays in the Time dim); `build()` runs a coverage-first portfolio (max coverage, then green→CO₂ / fast→finish / balanced→TND cost), cheap by default (balanced/fast escalate only on a serviceable drop; `coverage_portfolio` flag). The DEFAULT plan no longer drops a serviceable delivery; green CO₂ ≤ fast (equal on this workbook — full coverage forces the parallel plan). `tests/test_coverage_portfolio.py` (5 tests). _Environment note: the venv under OneDrive intermittently reverts to Python 3.11 while packages are cp313; ran the suite via the system 3.13 interpreter against the venv site-packages._
